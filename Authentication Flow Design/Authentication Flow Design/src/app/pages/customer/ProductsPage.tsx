...
function ProductCard({ product }: { product: (typeof PRODUCTS)[0] }) {
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group border border-gray-100 flex flex-col">
      <Link to={`/customer/product/${product.id}`} className="block relative overflow-hidden flex-shrink-0">
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="p-4 flex flex-col flex-1">
        <Link to={`/customer/product/${product.id}`} className="flex-1">
                  ...
                  <div key={item.id} className="flex items-start gap-2 p-2 bg-purple-50 rounded-lg border border-purple-100">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        <span className="text-[10px] font-bold text-[#d35f1a] flex-shrink-0">{formatPrice(item.price)}</span>
                      </div>
...
export default function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedWeights.length > 0) list = list.filter((p) => p.variants.some((v) => selectedWeights.includes(v.weight)));
    ...
    return list;
  }, [category, selectedWeights, sortBy]);
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-gray-500">
                    ...
                    {hasFilter && <button onClick={clearAll} className="ml-2 text-[#7C2D12] hover:underline text-xs">Xoá lọc</button>}
                  </p>
                </div>
              </div>
              <select
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-24 text-gray-500">
                ...
                <p className="text-sm text-gray-400 mb-4">Thử thay đổi bộ lọc hoặc tìm kiếm khác</p>
