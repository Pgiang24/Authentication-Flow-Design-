...
import { Search, Filter, X, ChevronLeft, ChevronRight, Eye, ChevronDown } from "lucide-react";
...
function OrderDrawer({ order, onClose, onStatusChange }: { order: Order; onClose: () => void; onStatusChange: (id: string, status: OrderStatus) => void }) {
  const [localStatus, setLocalStatus] = useState<OrderStatus>(order.status);

  const handleSave = () => {
    onStatusChange(order.id, localStatus);
    onClose();
  };
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
              ...
              <div>
                <div className="text-sm font-medium text-gray-900 capitalize">{order.paymentMethod === "bank" ? "Bank Transfer" : "Cash on Delivery"}</div>
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${paymentConfig[order.paymentStatus]}`}>
                  {order.paymentStatus}
                </span>
              </div>
            </div>
          ...
          <div>
            ...
            {localStatus !== order.status && (
              <div className="mt-2 text-xs text-amber-600 bg-amber-50 p-2 rounded-lg">
                ⚠ This will change the order status from "{statusConfig[order.status].label}" to "{statusConfig[localStatus].label}"
              </div>
            )}
          </div>
                  ...
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        <span className="text-xs text-gray-500 capitalize">{order.paymentMethod === "bank" ? "Bank Transfer" : "COD"}</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize w-fit ${paymentConfig[order.paymentStatus]}`}>
                          {order.paymentStatus}
                        </span>
                      </div>
                    </td>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={page === 1}
                className="p-2 rounded-lg border border-gray-200 text-gray-600 hover:border-[#7C2D12] hover:text-[#7C2D12] disabled:opacity-40 transition-all"
              >
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <button
                  key={p}
                  onClick={() => setPage(p)}
                  className={`w-8 h-8 rounded-lg text-sm font-medium transition-all ${page === p ? "bg-[#7C2D12] text-white" : "border border-gray-200 text-gray-600 hover:border-[#7C2D12]"}`}
                >
                  {p}
                </button>
              ))}
