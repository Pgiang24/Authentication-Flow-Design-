...
export default function CustomerLayout() {
  ...
  const navLinks = [
    { label: "Trang chủ",    to: "/customer" },
    { label: "Sản phẩm",     to: "/customer/products" },
    { label: "Về chúng tôi", to: "/customer#about" },
    { label: "Liên hệ",      to: "/customer#contact" },
  ];

  const isActive = (to: string) => {
    if (to === "/customer") return location.pathname === "/customer";
    if (to === "/customer/products") return location.pathname === "/customer/products";
    return false;
  };
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2]">
      ...
      <header className="sticky top-0 z-50 shadow-md">
        ...
        <div className="bg-white border-b border-gray-100">
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex items-center justify-between h-14">
              ...
              <Link to="/customer" className="flex items-center gap-2.5 flex-shrink-0">
                <div className="w-10 h-10 rounded-xl bg-[#7C2D12] flex items-center justify-center shadow-sm">
        ...
        {/* HÀNG 2: nền #d35f1a — Nav trái | Search+Phone+Cart+Profile phải */}
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                ...
                <Link to="/customer/cart" className="relative p-2 text-white/80 hover:text-white hover:bg-white/15 rounded-lg transition-colors">
                ...
                <div className="relative">
                  <button onClick={() => setProfileMenuOpen(!profileMenuOpen)} className="flex items-center gap-1 p-1.5 hover:bg-white/15 rounded-lg transition-colors">
          ...
          <div className="sm:hidden bg-white border-t border-gray-100">
            {navLinks.map((link) => (
              <Link
                ...
                className={`block px-6 py-3 text-sm font-medium border-b border-gray-50 ${isActive(link.to) ? "text-[#7C2D12] bg-[#7C2D12]/5" : "text-gray-700"}`}
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            <div>
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 rounded-lg bg-[#7C2D12] flex items-center justify-center">🏔️</div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-gray-400">
                <li><Link to="/customer/products?category=buffalo" className="hover:text-[#D4A853]">Thịt gác bếp</Link></li>
                <li><Link to="/customer/products?category=pork"    className="hover:text-[#D4A853]">Thịt hun khói</Link></li>
                <li><Link to="/customer/products?category=sausage" className="hover:text-[#D4A853]">Gia vị</Link></li>
                <li><Link to="/customer/products?category=poultry" className="hover:text-[#D4A853]">Gia cầm</Link></li>
              </ul>
            </div>
            <div>
