...
import { MOCK_INVENTORY, InventoryItem } from "../../data/adminData";
...
export default function InventoryPage() {
  ...
  const lowStockItems = inventory.flatMap((item) =>
    item.variants.filter((v) => v.stock > 0 && v.stock <= LOW_STOCK_THRESHOLD)
  ).length;
  const outOfStockItems = inventory.flatMap((item) =>
    item.variants.filter((v) => v.stock === 0)
  ).length;
              ...
              {inventory.map((item) =>
                item.variants.map((variant, vi) => {
                  ...
                  return (
                    <tr
                      key={`${item.id}-${vi}`}
                      className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${variant.stock === 0 ? "bg-red-50/30" : variant.stock <= LOW_STOCK_THRESHOLD ? "bg-amber-50/30" : ""}`}
                    >
                      {vi === 0 && (
